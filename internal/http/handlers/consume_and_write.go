...
func consumeAndWrite(hub HubInterface, userID string, ctx context.Context) {
	...
	// сначала проверим PEL и обработаем их
		...
		default:
			...
			for _, s := range streams {
				for _, msg := range s.Messages {
					...
					if err != nil {
						log.Println("consumeAndWrite processMessage error:", err)
					}
				}
			}
		}
